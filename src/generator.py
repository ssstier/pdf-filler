        ...
        for annotation in annotations:
            ...
            if annotation['/Subtype'] == '/Widget':
                ...
                if ft == 'radio':
                    ...
                    choices = [each[1].to_unicode() for each in
                               annotation['/Opt']]
                    values = []
                    for each in annotation['/Opt']:
                        if each[0] in value:
...
def single_form_fill(in_file, data, out_file, open_after=False):
...
def batch_form_fill(template, data, out_file, double_sided=False, splice=-1):
    ...
    for idx, record in enumerate(data):
        pdf = pdfrw.PdfReader(
            fdata=pdf_data)  # Create a new instance each time
    ...
    writer.trailer.Root.AcroForm = pdfrw.PdfReader(
        fdata=pdf_data).Root.AcroForm
    writer.trailer.Root.AcroForm.update(
        pdfrw.PdfDict(NeedAppearances=pdfrw.PdfObject('true')))
